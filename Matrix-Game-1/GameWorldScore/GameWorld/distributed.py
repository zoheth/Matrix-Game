...
def dist_init():
    ...
    if 'MASTER_PORT' not in os.environ:
        ...
        random_port = random.randint(25000, 30000)
...
def all_gather(data):
    ...
    if origin_size is not None:
        ...
        return data_list # ["Result from GPU 0", "Result from GPU 1", ..., "Result from GPU N"]
...
def merge_list_of_list(results): # 展平二维列表
    results = [item for sublist in results for item in sublist]
    return results


def gather_list_of_dict(results): # 聚合所有进程的数据并展平
    results = all_gather(results)
    results = merge_list_of_list(results)
    return results


def distribute_list_to_rank(data_list): # 将任务平均分配到各个 GPU 处理
