...
def test_indexing():
    ...
    # Create test data
    hidden_states = torch.randn(BS, T_q, C_hidden, device='cuda')
    mouse_condition = torch.randn(B, N_frames, C_mouse, device='cuda')

    # PyTorch reference
    preprocessor = MousePreprocessor(V, W)
    expected = preprocessor.forward(hidden_states, mouse_condition, is_causal=False, num_frame_per_block=-1)

    # Triton implementation
    actual = mouse_preprocessor_triton(hidden_states, mouse_condition, V, W, is_causal=False)
    ...
    hidden_part_match = torch.allclose(expected[:, :, :C_hidden], actual[:, :, :C_hidden])
    print(f"\nHidden states match: {hidden_part_match}")

    # Compare mouse part
    mouse_part_expected = expected[:, :, C_hidden:]
    mouse_part_actual = actual[:, :, C_hidden:]
...
def test_manual_verification():
    ...
    BS, T_q, C_hidden = 2, 3, 4
    V, W = 4, 2

    # Create simple test data with known values
    hidden_states = torch.ones(BS, T_q, C_hidden, device='cuda')
    ...
    for t in range(T_q):
        ...
        mouse_slice_actual = actual[0, t, C_hidden:C_hidden + C_mouse]
...
def test_real_inference_shapes():
    ...
    mouse_match = torch.allclose(expected[:, :, C_hidden:], actual[:, :, C_hidden:], rtol=1e-3, atol=1e-3)
